import type { TranslationSchema } from './en-US'

export const deDE: TranslationSchema = {
	ext: {
		status: {
			ready: 'Bereit',
			running: 'Läuft',
			completed: 'Fertig',
			error: 'Fehler',
			stopped: 'Gestoppt',
		},
		header: {
			history: 'Verlauf',
			settings: 'Einstellungen',
			back: 'Zurück',
			newChat: 'Neuer Chat',
			menu: 'Menü',
		},
		menu: { skills: 'Skills', settings: 'Einstellungen' },
		skills: {
			title: 'Skills',
			description:
				'Wiederverwendbare, parametrisierte Automationen, die der Agent auf passenden Seiten ausführen kann.',
			teachHint:
				'Nimm einen neuen Skill per Sprache mit der Lehren-Schaltfläche im Seitenpanel auf.',
			empty:
				'Noch keine Skills. Verfeinere unten einen aus Text oder nutze die Lehren-Schaltfläche im Seitenpanel.',
			add: 'Skill hinzufügen',
			name: 'Name',
			namePlaceholder: 'z. B. checkout_express',
			urlPattern: 'URL-Muster',
			urlPatternPlaceholder: 'z. B. *example.com/*',
			skillDescription: 'Beschreibung',
			skillDescriptionPlaceholder: 'Was er tut und wann er zu verwenden ist',
			plan: 'Schritte',
			planPlaceholder: 'Geordnete Schritte des Agenten; nutze {param} für Eingaben',
			parameters: 'Parameter',
			paramName: 'Name',
			paramDescription: 'Beschreibung',
			paramRequired: 'Erforderlich',
			addParam: 'Parameter hinzufügen',
			removeParam: 'Parameter entfernen',
			enabled: 'Aktiviert',
			delete: 'Skill löschen',
			refineFromText: 'Aus Text verfeinern',
			refineHint: 'Füge eine Erzählung ein oder tippe sie; die KI strukturiert sie zu einem Skill.',
			transcriptPlaceholder: 'Beschreibe die Schritte in einfacher Sprache…',
			refine: 'Verfeinern',
			refining: 'Wird verfeinert…',
			reRefine: 'Neu verfeinern',
			refined: 'Skill verfeinert.',
			refineFailed: 'Verfeinerung fehlgeschlagen',
			refineNeedsLlm: 'Konfiguriere zuerst einen KI-Anbieter.',
			needNarration: 'Gib zuerst eine Erzählung ein.',
			noTranscript: 'Keine gespeicherte Erzählung zum Verfeinern.',
		},
		teach: {
			open: 'Skill lehren',
			title: 'Skill lehren',
			hint: 'Erzähle die Schritte, während du die Seite ansiehst.',
			record: 'Aufnehmen',
			stop: 'Stopp',
			transcribing: 'Wird transkribiert…',
			transcript: 'Transkript',
			transcriptPlaceholder: 'Deine Erzählung erscheint hier – bei Bedarf bearbeiten.',
			noVoiceHint:
				'Sprache ist nicht eingerichtet. Tippe die Schritte unten oder aktiviere Sprache in den Einstellungen.',
			refine: 'Verfeinern',
			refining: 'Wird verfeinert…',
			review: 'Skill prüfen und bearbeiten, dann speichern.',
			save: 'Skill speichern',
			discard: 'Verwerfen',
			close: 'Schließen',
			saved: 'Skill gespeichert – verwalte ihn unter Einstellungen › Skills.',
			needTranscript: 'Füge zuerst eine Erzählung hinzu.',
			refineFailed: 'Verfeinerung fehlgeschlagen',
		},
		masking: {
			title: 'Gespeicherte Daten & Maskierung',
			description:
				'Speichere Werte, die der Agent in Formulare einfügen kann. Markiere einen Wert als sensibel, damit er nie den KI-Anbieter erreicht – der Agent sieht nur ein anonymes {{token}}.',
			unencryptedNote:
				'Werte werden unverschlüsselt in diesem Browser gespeichert, wie deine API-Schlüssel.',
			jsDisabledNote:
				'Solange ein Eintrag aktiv ist, wird das experimentelle JavaScript-Tool deaktiviert, um maskierte Werte zu schützen.',
			empty: 'Noch keine gespeicherten Daten. Füge einen Eintrag oder eine Adressgruppe hinzu.',
			label: 'Bezeichnung',
			labelPlaceholder: 'z. B. Primäre Kreditkarte',
			token: 'Token',
			tokenPlaceholder: 'z. B. credit_card',
			value: 'Wert',
			valuePlaceholder: 'Der echte Wert',
			group: 'Gruppe',
			groupPlaceholder: 'z. B. Heimatadresse',
			sensitive: 'Sensibel',
			sensitiveHelp:
				'Sensible Werte werden aus dem Seiteninhalt entfernt und nie an die KI gesendet. Nicht sensible Werte (Name, Adresse) werden automatisch ausgefüllt und können von der KI gesehen werden.',
			enabled: 'Aktiviert',
			delete: 'Eintrag löschen',
			showValue: 'Wert anzeigen',
			hideValue: 'Wert verbergen',
			addEntry: 'Eintrag hinzufügen',
			addAddressGroup: 'Adressgruppe hinzufügen',
			addressGroupName: 'Heimatadresse',
		},
		voice: {
			title: 'Stimme',
			description:
				'Sprich mit dem Agenten und höre seine Antworten. Wähle Anbieter für Sprache-zu-Text und Text-zu-Sprache; die Web Speech API funktioniert ohne API-Schlüssel.',
			enabled: 'Sprachmodus aktivieren',
			enabledHelp: 'Die Mikrofon-Schaltfläche anzeigen und Push-to-Talk aktivieren.',
			micAccess: 'Mikrofonzugriff',
			micAccessHelp:
				'Spracheingabe benötigt das Mikrofon. Hier erteilen – das Seitenpanel kann nicht direkt danach fragen.',
			micGrant: 'Zugriff erteilen',
			micGranted: 'Mikrofonzugriff erteilt.',
			micDenied:
				'Mikrofonzugriff ist blockiert. Aktiviere ihn in den Browser-Website-Einstellungen für diese Erweiterung und lade dann neu.',
			micCheck: 'Erneut prüfen',
			missingKey:
				'Füge einen API-Schlüssel für die gewählten Sprachanbieter hinzu, um zu speichern.',
			sttHeading: 'Sprache zu Text',
			ttsHeading: 'Text zu Sprache',
			interactionHeading: 'Interaktion',
			provider: 'Anbieter',
			model: 'Modell',
			voice: 'Stimme',
			voicePlaceholder: 'Standardstimme',
			noVoices: 'Keine Stimmen gefunden',
			apiKey: 'API-Schlüssel',
			reusingChatKey: 'Dein Chat-Anbieter-API-Schlüssel wird wiederverwendet.',
			autoSpeak: 'Antworten laut vorlesen',
			pushToTalk: 'Zum Sprechen halten',
			pushToTalkHelp: 'Eine Taste zum Aufnehmen halten; zum Senden loslassen.',
			holdKey: 'Halte-Taste',
			holdKeyHelp:
				'Taste, die beim Sprechen gehalten wird (z. B. Alt). Funktioniert auf der Seite und im Panel.',
			language: 'Sprache',
			languagePlaceholder: 'Auto (z. B. en, tr)',
			test: 'Stimme testen',
			testSuccess: 'Eine Testphrase abgespielt.',
			testPhrase: 'Hallo, dies ist ein Navvy-Sprachtest.',
			testNoController: 'Konfiguriere zuerst einen Anbieter.',
		},
		empty: {
			tip1: 'Überlasse die langweiligen Klicks der KI',
			tip2: 'Surfe mit Absicht, nicht mit der Maus',
			tip3: 'Mache jede Website zugänglicher',
			tip4: 'Mehr denken, weniger klicken',
			github: 'GitHub',
			docs: 'Dokumentation',
			website: 'Webseite',
		},
		task: { label: 'Aufgabe' },
		input: {
			placeholder: 'Beschreiben Sie Ihre Aufgabe... (Enter zum Senden)',
			send: 'Senden',
			stop: 'Aufgabe stoppen',
			voice: {
				start: 'Spracheingabe starten',
				stop: 'Stoppen und transkribieren',
				transcribing: 'Wird transkribiert…',
				listening: 'Höre zu…',
				disabled: 'Stimme in den Einstellungen aktivieren',
				noSpeech: 'Keine Sprache erkannt – bitte erneut versuchen.',
				failed: 'Spracheingabe fehlgeschlagen',
			},
			attach: {
				label: 'Anhängen',
				unsupported: 'Das ausgewählte Modell unterstützt keine Bilder',
				captureTab: 'Aktiven Tab erfassen',
				uploadImage: 'Bild hochladen',
				remove: 'Bild entfernen',
				thumbAlt: 'Angehängtes Bild',
				captureFailed: 'Tab-Screenshot fehlgeschlagen',
				uploadFailed: 'Bilddatei konnte nicht gelesen werden',
			},
		},
		history: {
			title: 'Verlauf',
			clearAll: 'Alle löschen',
			noHistory: 'Noch kein Verlauf',
			loading: 'Lädt...',
			justNow: 'gerade eben',
			minutesAgo: 'vor {{n}} Min.',
			hoursAgo: 'vor {{n}} Std.',
			daysAgo: 'vor {{n}} T.',
			stepsCount: '{{n}} Schritte',
			runAgain: 'Erneut ausführen',
			runAgainTitle: 'Aufgabe erneut ausführen',
			exportTitle: 'Verlauf als JSON exportieren',
			deleteTitle: 'Verlauf löschen',
			delete: 'Löschen',
		},
		config: {
			title: 'Einstellungen',
			userAuthToken: 'Benutzer-Auth-Token',
			userAuthTokenHelp: 'Erlaubt einer Webseite, diese Erweiterung aufzurufen.',
			loading: 'Lädt...',
			showToken: 'Token anzeigen',
			hideToken: 'Token verbergen',
			copyToken: 'Token kopieren',
			tokenCopied: 'Token kopiert',
			manageHub: 'Navvy Hub verwalten',
			profile: 'Profil',
			profileAdd: 'Profil hinzufügen',
			profileDelete: 'Profil löschen',
			profileNewName: 'Neues Profil',
			profileUnnamed: 'Unbenannt',
			profileNamePlaceholder: 'Profilname',
			provider: 'Anbieter',
			getApiKey: 'API-Schlüssel erhalten',
			baseUrl: 'Basis-URL',
			model: 'Modell',
			apiKey: 'API-Schlüssel',
			showApiKey: 'API-Schlüssel anzeigen',
			hideApiKey: 'API-Schlüssel verbergen',
			accountId: 'Konto-ID',
			accountIdPlaceholder: 'z. B. 1a2b3c4d5e6f7g8h9i0j',
			accountIdHelp: 'Konto-ID finden',
			modelDisabledApiKey: 'API-Schlüssel eingeben, um ein Modell auszuwählen.',
			modelDisabledAccountId: 'Konto-ID und API-Schlüssel eingeben, um ein Modell auszuwählen.',
			modelDisabledBaseUrl: 'Basis-URL eingeben, um ein Modell auszuwählen.',
			modelNoMatch: 'Keine passenden Modelle',
			addAnotherProfile: 'Weitere Konfiguration hinzufügen',
			profileName: 'Diese Konfiguration benennen',
			profileNameHelp: 'Eine Bezeichnung, um gespeicherte Konfigurationen zu unterscheiden.',
			profileDeleteConfirmTitle: 'Diese Konfiguration löschen?',
			profileDeleteConfirmBody:
				'"{{name}}" und alle gespeicherten API-Schlüssel werden dauerhaft von diesem Gerät entfernt. Dies kann nicht rückgängig gemacht werden.',
			responseLanguage: 'Antwortsprache',
			languageSystem: 'System',
			advanced: 'Erweitert',
			maxSteps: 'Max. Schritte',
			systemInstruction: 'Systemanweisung',
			systemInstructionPlaceholder: 'Zusätzliche Anweisungen für den Agenten...',
			disableNamedToolChoice: 'Benannte tool_choice deaktivieren',
			expLlmsTxt: 'Experimentelle llms.txt-Unterstützung',
			expIncludeAllTabs: 'Alle Tabs einbeziehen (experimentell)',
			cancel: 'Abbrechen',
			save: 'Speichern',
			version: 'Version',
			sourceCode: 'Quellcode',
			homePage: 'Startseite',
			privacy: 'Datenschutz',
			testingApiNotice: 'Sie verwenden unsere Test-API. Mit der Nutzung akzeptieren Sie die',
			termsAndPrivacy: 'Nutzungsbedingungen und Datenschutzerklärung',
			testConnection: 'Verbindung testen',
			testConnectionRunning: 'Wird getestet…',
			testConnectionSuccess: 'Verbindung erfolgreich',
			testConnectionMissingApiKey: 'Bitte zuerst einen API-Schlüssel eingeben.',
			testConnectionMissingAccountId: 'Bitte zuerst eine Account-ID eingeben.',
			testConnectionMissingModel: 'Bitte zuerst ein Modell auswählen.',
			testConnectionMissingBaseUrl: 'Bitte zuerst eine Basis-URL eingeben.',
			testConnectionFailed: 'Fehlgeschlagen: {{message}}',
		},
		settings: {
			tabGeneral: 'Allgemein',
			tabProviders: 'Anbieter',
			tabMasking: 'Gespeicherte Daten',
			tabVoice: 'Stimme',
			tabSkills: 'Fähigkeiten',
			tabAdvanced: 'Erweitert',
			tabAbout: 'Über',
			uiLanguage: 'Oberflächensprache',
			uiLanguageHelp: 'Sprache der Benutzeroberfläche der Erweiterung.',
			responseLanguage: 'Antwortsprache',
			responseLanguageHelp:
				'Sprache, in der der Agent antwortet. „Auto“ passt sich der Sprache deiner Aufgabe an.',
			languageAuto: 'Auto',
			unsavedNotice: 'Du hast nicht gespeicherte Änderungen',
			savedNotice: 'Gespeichert',
			close: 'Schließen',
		},
		error: {
			title: 'Etwas ist schiefgelaufen',
			unexpected: 'Ein unerwarteter Fehler ist aufgetreten',
			resetConfig: 'Konfig. zurücksetzen',
			reloadPanel: 'Panel neu laden',
		},
		cards: {
			resultLabel: 'Ergebnis',
			resultSuccess: 'Erfolg',
			resultFailed: 'Fehlgeschlagen',
			step: 'Schritt',
			actions: 'Aktionen',
			rawRequest: 'Rohanfrage',
			rawResponse: 'Rohantwort',
			copy: 'Kopieren',
			copied: 'Kopiert!',
			copySystem: 'System kopieren',
			copyUser: 'Benutzer kopieren',
			hintQuotaExceeded:
				'Das Kontingent oder Guthaben Ihres Anbieters scheint aufgebraucht zu sein.',
			hintAuthFailed: 'Ihr API-Schlüssel ist möglicherweise ungültig oder abgelaufen.',
			openProviderSettings: 'Anbieter-Einstellungen öffnen',
		},
		activity: {
			thinking: 'Denke nach...',
			executing: 'Führe {{tool}} aus...',
			done: 'Fertig: {{tool}}',
			retrying: 'Erneuter Versuch ({{attempt}}/{{max}})...',
		},
		hub: {
			name: 'Navvy Hub',
			beta: 'Beta',
			intro:
				'Navvy Hub erlaubt lokalen Apps (z. B. MCP-Servern), die Navvy-Erweiterung über WebSocket zu steuern.',
			checkOutOfficial: 'Sehen Sie sich das offizielle',
			mcpServerPackage: 'MCP-Server-Paket',
			configHeading: 'Konfiguration',
			autoApprove: 'Verbindungen automatisch genehmigen',
			autoApproveHelp1:
				'Standardmäßig erfordert jede Verbindung Ihre Bestätigung, bevor Aufgaben ausgeführt werden.',
			autoApproveHelp2: 'Aktivieren Sie dies, um die Bestätigung pro Sitzung zu überspringen.',
			autoApproveCaution: '* Mit Vorsicht verwenden!',
			docsHeading: 'Docs',
			connectVia: 'Verbinden über',
			flowHeading: 'Ablauf',
			callerToHub: 'Aufrufer → Hub',
			hubToCaller: 'Hub → Aufrufer',
			connected: 'Verbunden',
			connecting: 'Verbinde…',
			disconnected: 'Getrennt',
			noConnection: 'Keine Verbindung',
			waitingForTask: 'Warte auf Aufgabe von externem Aufrufer…',
			noActiveSession: 'Keine aktive Sitzung',
			currentTask: 'Aktuelle Aufgabe',
			stop: 'Stopp',
		},
	},
}
