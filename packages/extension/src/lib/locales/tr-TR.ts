import type { TranslationSchema } from './en-US'

export const trTR: TranslationSchema = {
	ext: {
		status: {
			ready: 'Hazır',
			running: 'Çalışıyor',
			completed: 'Bitti',
			error: 'Hata',
			stopped: 'Durduruldu',
		},
		header: {
			history: 'Geçmiş',
			settings: 'Ayarlar',
			back: 'Geri',
			newChat: 'Yeni sohbet',
			menu: 'Menü',
		},
		menu: { skills: 'Beceriler', settings: 'Ayarlar' },
		skills: {
			title: 'Beceriler',
			description:
				'Ajanın eşleşen sayfalarda çalıştırabileceği, yeniden kullanılabilir ve parametreli otomasyonlar.',
			teachHint: 'Yan paneldeki Öğret düğmesiyle sesle yeni bir beceri kaydedin.',
			empty:
				'Henüz beceri yok. Aşağıda metinden bir tane oluşturun ya da yan paneldeki Öğret düğmesini kullanın.',
			add: 'Beceri ekle',
			name: 'Ad',
			namePlaceholder: 'örn. checkout_express',
			urlPattern: 'URL deseni',
			urlPatternPlaceholder: 'örn. *example.com/*',
			skillDescription: 'Açıklama',
			skillDescriptionPlaceholder: 'Ne yaptığı ve ne zaman kullanılacağı',
			plan: 'Adımlar',
			planPlaceholder: 'Ajanın izlediği sıralı adımlar; girdiler için {param} kullanın',
			parameters: 'Parametreler',
			paramName: 'Ad',
			paramDescription: 'Açıklama',
			paramRequired: 'Zorunlu',
			addParam: 'Parametre ekle',
			removeParam: 'Parametreyi kaldır',
			enabled: 'Etkin',
			delete: 'Beceriyi sil',
			refineFromText: 'Metinden oluştur',
			refineHint: 'Bir anlatım yapıştırın veya yazın; yapay zeka bunu bir beceriye dönüştürür.',
			transcriptPlaceholder: 'Adımları sade bir dille anlatın…',
			refine: 'Oluştur',
			refining: 'Oluşturuluyor…',
			reRefine: 'Yeniden oluştur',
			refined: 'Beceri oluşturuldu.',
			refineFailed: 'Oluşturma başarısız',
			refineNeedsLlm: 'Önce bir yapay zeka sağlayıcısı yapılandırın.',
			needNarration: 'Önce bir anlatım girin.',
			noTranscript: 'Oluşturmak için kayıtlı anlatım yok.',
		},
		teach: {
			open: 'Beceri öğret',
			title: 'Beceri öğret',
			hint: 'Sayfaya bakarken adımları anlatın.',
			record: 'Kaydet',
			stop: 'Durdur',
			transcribing: 'Yazıya dökülüyor…',
			transcript: 'Döküm',
			transcriptPlaceholder: 'Anlatımınız burada görünür — gerekirse düzenleyin.',
			noVoiceHint: 'Ses ayarlanmamış. Adımları aşağıya yazın veya ayarlardan sesi etkinleştirin.',
			refine: 'Oluştur',
			refining: 'Oluşturuluyor…',
			review: 'Beceriyi gözden geçirip düzenleyin, sonra kaydedin.',
			save: 'Beceriyi kaydet',
			discard: 'Vazgeç',
			close: 'Kapat',
			saved: 'Beceri kaydedildi — Ayarlar › Beceriler’den yönetin.',
			needTranscript: 'Önce bir anlatım ekleyin.',
			refineFailed: 'Oluşturma başarısız',
		},
		masking: {
			title: 'Kaydedilen Veriler ve Maskeleme',
			description:
				'Ajanın formlara doldurabileceği değerleri kaydedin. Bir değeri hassas olarak işaretleyin; böylece asla yapay zeka sağlayıcısına ulaşmaz — ajan yalnızca opak bir {{token}} görür.',
			unencryptedNote: 'Değerler, API anahtarlarınız gibi bu tarayıcıda şifrelenmeden saklanır.',
			jsDisabledNote:
				'Herhangi bir kayıt etkinken, maskelenmiş değerleri korumak için deneysel JavaScript aracı devre dışı bırakılır.',
			empty: 'Henüz kaydedilmiş veri yok. Başlamak için bir kayıt veya adres grubu ekleyin.',
			label: 'Etiket',
			labelPlaceholder: 'ör. Ana kredi kartı',
			token: 'Token',
			tokenPlaceholder: 'ör. credit_card',
			value: 'Değer',
			valuePlaceholder: 'Gerçek değer',
			group: 'Grup',
			groupPlaceholder: 'ör. Ev adresi',
			sensitive: 'Hassas',
			sensitiveHelp:
				'Hassas değerler sayfa içeriğinden çıkarılır ve asla yapay zekaya gönderilmez. Hassas olmayan değerler (ad, adres) otomatik doldurulur ve yapay zeka tarafından görülebilir.',
			enabled: 'Etkin',
			delete: 'Kaydı sil',
			showValue: 'Değeri göster',
			hideValue: 'Değeri gizle',
			addEntry: 'Kayıt ekle',
			addAddressGroup: 'Adres grubu ekle',
			addressGroupName: 'Ev adresi',
		},
		voice: {
			title: 'Ses',
			description:
				'Ajanla konuşun ve yanıtlarını dinleyin. Konuşmadan metne ve metinden konuşmaya sağlayıcılarını seçin; Web Speech API herhangi bir API anahtarı olmadan çalışır.',
			enabled: 'Ses modunu etkinleştir',
			enabledHelp: 'Mikrofon düğmesini göster ve bas-konuş özelliğini etkinleştir.',
			micAccess: 'Mikrofon erişimi',
			micAccessHelp:
				'Sesli giriş mikrofon gerektirir. Buradan izin ver — yan panel doğrudan isteyemez.',
			micGrant: 'Erişim ver',
			micGranted: 'Mikrofon erişimi verildi.',
			micDenied:
				'Mikrofon erişimi engellenmiş. Tarayıcının bu uzantı için site ayarlarından etkinleştir, ardından yeniden yükle.',
			micCheck: 'Tekrar kontrol et',
			missingKey: 'Kaydetmek için seçili ses sağlayıcıları için bir API anahtarı ekleyin.',
			sttHeading: 'Konuşmadan metne',
			ttsHeading: 'Metinden konuşmaya',
			interactionHeading: 'Etkileşim',
			provider: 'Sağlayıcı',
			model: 'Model',
			voice: 'Ses',
			voicePlaceholder: 'Varsayılan ses',
			noVoices: 'Ses bulunamadı',
			apiKey: 'API anahtarı',
			reusingChatKey: 'Sohbet sağlayıcınızın API anahtarı yeniden kullanılıyor.',
			autoSpeak: 'Yanıtları sesli oku',
			pushToTalk: 'Konuşmak için basılı tut',
			pushToTalkHelp: 'Kaydetmek için bir tuşu basılı tutun; göndermek için bırakın.',
			holdKey: 'Basılı tutma tuşu',
			holdKeyHelp: 'Konuşurken basılı tutulacak tuş (örn. Alt). Sayfada ve panelde çalışır.',
			language: 'Dil',
			languagePlaceholder: 'Otomatik (örn. en, tr)',
			test: 'Sesi test et',
			testSuccess: 'Bir test ifadesi çalındı.',
			testPhrase: 'Merhaba, bu bir Navvy ses testidir.',
			testNoController: 'Önce bir sağlayıcı yapılandırın.',
		},
		empty: {
			tip1: 'Sıkıcı tıklamaları yapay zekâya bırak',
			tip2: 'Fareyle değil, niyetle gez',
			tip3: 'Her siteyi daha erişilebilir kıl',
			tip4: 'Daha az tıkla, daha çok düşün',
			github: 'GitHub',
			docs: 'Belgeler',
			website: 'Web sitesi',
		},
		task: { label: 'Görev' },
		input: {
			placeholder: 'Görevinizi yazın... (göndermek için Enter)',
			send: 'Gönder',
			stop: 'Görevi durdur',
			voice: {
				start: 'Sesli girişi başlat',
				stop: 'Durdur ve deşifre et',
				transcribing: 'Deşifre ediliyor…',
				listening: 'Dinleniyor…',
				disabled: 'Sesi ayarlardan etkinleştir',
				noSpeech: 'Konuşma algılanmadı — tekrar deneyin.',
				failed: 'Sesli giriş başarısız',
			},
			attach: {
				label: 'Ekle',
				unsupported: 'Seçili model resim desteklemiyor',
				captureTab: 'Aktif sekmenin ekran görüntüsü',
				uploadImage: 'Resim yükle',
				remove: 'Resmi kaldır',
				thumbAlt: 'Eklenen resim',
				captureFailed: 'Sekme ekran görüntüsü alınamadı',
				uploadFailed: 'Resim dosyası okunamadı',
			},
		},
		history: {
			title: 'Geçmiş',
			clearAll: 'Tümünü temizle',
			noHistory: 'Geçmiş yok',
			loading: 'Yükleniyor...',
			justNow: 'az önce',
			minutesAgo: '{{n}} dk önce',
			hoursAgo: '{{n}} sa önce',
			daysAgo: '{{n}} gün önce',
			stepsCount: '{{n}} adım',
			runAgain: 'Tekrar çalıştır',
			runAgainTitle: 'Görevi tekrar çalıştır',
			exportTitle: 'Geçmişi JSON olarak dışa aktar',
			deleteTitle: 'Geçmişi sil',
			delete: 'Sil',
		},
		config: {
			title: 'Ayarlar',
			userAuthToken: 'Kullanıcı kimlik doğrulama belirteci',
			userAuthTokenHelp: 'Bir web sitesinin bu eklentiyi çağırmasına izin verir.',
			loading: 'Yükleniyor...',
			showToken: 'Belirteci göster',
			hideToken: 'Belirteci gizle',
			copyToken: 'Belirteci kopyala',
			tokenCopied: 'Belirteç kopyalandı',
			manageHub: 'Navvy Hub’ı yönet',
			profile: 'Profil',
			profileAdd: 'Profil ekle',
			profileDelete: 'Profili sil',
			profileNewName: 'Yeni profil',
			profileUnnamed: 'Adsız',
			profileNamePlaceholder: 'Profil adı',
			provider: 'Sağlayıcı',
			getApiKey: 'API anahtarı al',
			baseUrl: 'Temel URL',
			model: 'Model',
			apiKey: 'API anahtarı',
			showApiKey: 'API anahtarını göster',
			hideApiKey: 'API anahtarını gizle',
			accountId: 'Hesap kimliği',
			accountIdPlaceholder: 'örn. 1a2b3c4d5e6f7g8h9i0j',
			accountIdHelp: 'Hesap kimliğini bul',
			modelDisabledApiKey: 'Model seçmek için API anahtarınızı girin.',
			modelDisabledAccountId: 'Model seçmek için hesap kimliğinizi ve API anahtarınızı girin.',
			modelDisabledBaseUrl: 'Model seçmek için bir temel URL girin.',
			modelNoMatch: 'Eşleşen model yok',
			addAnotherProfile: 'Başka bir yapılandırma ekle',
			profileName: 'Bu yapılandırmaya isim ver',
			profileNameHelp: 'Kayıtlı yapılandırmalarını birbirinden ayırt etmek için bir etiket.',
			profileDeleteConfirmTitle: 'Bu yapılandırma silinsin mi?',
			profileDeleteConfirmBody:
				'"{{name}}" ve kayıtlı tüm API anahtarları bu cihazdan kalıcı olarak kaldırılacak. Bu işlem geri alınamaz.',
			responseLanguage: 'Yanıt dili',
			languageSystem: 'Sistem',
			advanced: 'Gelişmiş',
			maxSteps: 'Maks. adım',
			systemInstruction: 'Sistem talimatı',
			systemInstructionPlaceholder: 'Ajan için ek talimatlar...',
			disableNamedToolChoice: 'Adlandırılmış tool_choice’u devre dışı bırak',
			expLlmsTxt: 'Deneysel llms.txt desteği',
			expIncludeAllTabs: 'Tüm sekmeleri dahil et (deneysel)',
			cancel: 'İptal',
			save: 'Kaydet',
			version: 'Sürüm',
			sourceCode: 'Kaynak kod',
			homePage: 'Ana sayfa',
			privacy: 'Gizlilik',
			testingApiNotice: 'Test API’mizi kullanıyorsunuz. Kullanarak şu koşulları kabul edersiniz:',
			termsAndPrivacy: 'Kullanım Koşulları ve Gizlilik Politikası',
			testConnection: 'Bağlantıyı test et',
			testConnectionRunning: 'Test ediliyor…',
			testConnectionSuccess: 'Bağlantı başarılı',
			testConnectionMissingApiKey: 'Önce bir API anahtarı girin.',
			testConnectionMissingAccountId: 'Önce bir hesap kimliği girin.',
			testConnectionMissingModel: 'Önce bir model seçin.',
			testConnectionMissingBaseUrl: 'Önce bir Base URL girin.',
			testConnectionFailed: 'Başarısız: {{message}}',
		},
		settings: {
			tabGeneral: 'Genel',
			tabProviders: 'Sağlayıcılar',
			tabMasking: 'Kaydedilen Veriler',
			tabVoice: 'Ses',
			tabSkills: 'Beceriler',
			tabAdvanced: 'Gelişmiş',
			tabAbout: 'Hakkında',
			uiLanguage: 'Arayüz dili',
			uiLanguageHelp: 'Eklenti arayüzünde kullanılan dil.',
			responseLanguage: 'Yanıt dili',
			responseLanguageHelp:
				'Ajanın yanıt verirken kullandığı dil. Otomatik, görevinizin dilini kullanır.',
			languageAuto: 'Otomatik',
			unsavedNotice: 'Kaydedilmemiş değişiklikleriniz var',
			savedNotice: 'Kaydedildi',
			close: 'Kapat',
		},
		error: {
			title: 'Bir şeyler ters gitti',
			unexpected: 'Beklenmeyen bir hata oluştu',
			resetConfig: 'Ayarları sıfırla',
			reloadPanel: 'Paneli yenile',
		},
		cards: {
			resultLabel: 'Sonuç',
			resultSuccess: 'Başarılı',
			resultFailed: 'Başarısız',
			step: 'Adım',
			actions: 'Eylemler',
			rawRequest: 'Ham istek',
			rawResponse: 'Ham yanıt',
			copy: 'Kopyala',
			copied: 'Kopyalandı!',
			copySystem: 'Sistemi kopyala',
			copyUser: 'Kullanıcıyı kopyala',
			hintQuotaExceeded: 'Sağlayıcı kotanız veya krediniz tükenmiş görünüyor.',
			hintAuthFailed: 'API anahtarınız geçersiz veya süresi dolmuş olabilir.',
			openProviderSettings: 'Sağlayıcı ayarlarını aç',
		},
		activity: {
			thinking: 'Düşünüyor...',
			executing: '{{tool}} çalıştırılıyor...',
			done: 'Tamam: {{tool}}',
			retrying: 'Yeniden deneniyor ({{attempt}}/{{max}})...',
		},
		hub: {
			name: 'Navvy Hub',
			beta: 'Beta',
			intro:
				'Navvy Hub, yerel uygulamaların (örn. MCP sunucuları) Navvy eklentisini WebSocket üzerinden kontrol etmesine olanak tanır.',
			checkOutOfficial: 'Resmi',
			mcpServerPackage: 'MCP sunucu paketine göz atın',
			configHeading: 'Yapılandırma',
			autoApprove: 'Bağlantıları otomatik onayla',
			autoApproveHelp1:
				'Varsayılan olarak her bağlantı, görevler çalıştırılmadan önce onayınızı gerektirir.',
			autoApproveHelp2: 'Oturum başına onayı atlamak için bunu etkinleştirin.',
			autoApproveCaution: '* Dikkatli kullanın!',
			docsHeading: 'Belgeler',
			connectVia: 'Bağlanmak için',
			flowHeading: 'Akış',
			callerToHub: 'Çağıran → Hub',
			hubToCaller: 'Hub → Çağıran',
			connected: 'Bağlı',
			connecting: 'Bağlanılıyor…',
			disconnected: 'Bağlantı kesildi',
			noConnection: 'Bağlantı yok',
			waitingForTask: 'Harici çağırandan görev bekleniyor…',
			noActiveSession: 'Etkin oturum yok',
			currentTask: 'Mevcut görev',
			stop: 'Durdur',
		},
	},
}
