...
(`packages/extension`). The four appendices add an architecture diagram, three
end-to-end worked examples, a design-decisions/trade-offs catalogue, a
limitations list, and a glossary — written for direct reuse in the report.
...
Each is a real engineering decision in the **Problem → Approach → Trade-off** shape. (See also
`docs/report/under-the-hood-catalog.md` for the input-shortcut history; the items below are
complementary.)
